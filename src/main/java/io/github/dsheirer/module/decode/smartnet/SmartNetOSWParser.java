...
public class SmartNetOSWParser
{
    ...
    private void parseQueuedOSWs()
    {
        ...
        // Individual voice channel grant: channel with individual flag (private call or radio command)
        ...
        // Status command to individual radio (affiliation, ack, etc.)
        if(!osw.isChannel && !osw.grp && osw.cmd != 0x2F8)
        {
            mOSWQueue.pollFirst();
            SmartNetMessage msg = new SmartNetMessage(osw.addr, osw.grp, osw.cmd,
                SmartNetMessageType.STATUS_ACK);
            dispatch(msg);
            return;
        }
